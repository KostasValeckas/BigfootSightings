        ...
        if form.validate_on_submit():
            ...
            if user and password == form.password.data:
                ...
                return (
                    redirect(next_page)
                    if next_page
                    else redirect(url_for("Login.backToMainFromLogin"))
                )
...
def signup():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if user is None and (form.password.data == form.password_repeat.data):
                ...
                if user:
                    ...
                    return (
                        redirect(next_page)
                        if next_page
                        else redirect(url_for("Login.backToMainFromSignup"))
                    )
