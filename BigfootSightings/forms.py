...
class UserLoginForm(FlaskForm):
    ...
    password = PasswordField(
        "Password", validators=[DataRequired()], render_kw=dict(placeholder="Password")
    )
...
class SearchSightingForm(FlaskForm):
    ...
    username = StringField(
        "Username",
        validators=[Length(min=0, max=5000)],
        render_kw=dict(placeholder="Search Text (case sensitive)"),
    )
    lat = StringField(
        "Latitude (+/- 1 degree)",
        validators=[Length(min=0, max=5000)],
    ...
    date = StringField(
        "Date",
        validators=[Length(min=0, max=5000)],
        render_kw=dict(placeholder="Format example: 2024-06-01"),
    )
    country = StringField(
        "Country",
        validators=[Length(min=0, max=5000)],
        render_kw=dict(placeholder="Search Text (Name in Local Language)"),
    )
    state = StringField(
        "State",
        validators=[Length(min=0, max=5000)],
...
class BackToMainForm(FlaskForm):
    submit = SubmitField("Main Menu")
